...
public abstract class FrameworkServlet extends HttpServletBean implements ApplicationContextAware {
	...
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac) {
		...
		//SourceFilteringListener 就是 #onApplicationEvent(ApplicationEvent event) 方法中，判断事件的来源，就是原始类 source(wac)。
		// 如果是，则调用 #onApplicationEventInternal(ApplicationEvent event) 方法，将事件转发给 delegate 监听器(ContextRefreshListener)
		//最终调用到 FrameworkServlet#onRefresh
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// <2.2> 调用父类，处理其它请求
		// 父类中service方法会调到 FrameworkServlet 中的实现，根据请求方法不同，处理略有不同，最终都是调用 processRequest 方法
		else {
	...
	protected final void processRequest(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			//抽象方法，由 DispatcherServlet 实现，为 DispatcherServlet 入口
